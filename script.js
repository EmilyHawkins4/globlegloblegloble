...
let targetCountry = countryArray[Math.floor(Math.random() * 35)];// only in americas
...
function checkInput() {
  ...
  for (var i = 0; i < countryArray.length; i++) {
    if (guess == countryArray[i]) {
      ...
      let distance = bfs(guess, targetCountry);
      addRow(guess, distance);
      addToMap(guess, distance)
      if (distance == 0) {
        winGame();
      }
    }
...
function addRow(guess, distance) {
  ...
  th2.innerHTML = distance + " borders";
  tr.appendChild(th2);
...
function addToMap(guess, distance) {
  let rgbVal = parseInt((255 / 20) * (distance))
  let color = `rgb(${rgbVal},${rgbVal},${0})`;
  // console.log(color)
