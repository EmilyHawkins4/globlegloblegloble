...
function pickColor(guess, correct) {

    var guessedNode = countryMap[guess];
    var realNode = countryMap[correct]

    // var borderDistance = minEdgeBFS(countryGraph, realNode, guessedNode, countryGraph.length);
    var borderDistance = bfs()

    console.log("distance: " + borderDistance);
...
function bfs() {
    ...
    while (Q.length != 0) {
        ...
        for (let i = 0; i < Q.length; i++) {
            ...
            visited.add(top)
            for (let country in countryGraph[top]) {
